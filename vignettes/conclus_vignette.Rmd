...
\newline
```{r normalization_2, results="hide", warning = FALSE}
sceObject <- conclus::normaliseCountMatrix(countMatrix, species = "mmu", 
                                  colData = columnsMetaData)
```
...
```
\newline
In the *clusterSimilarityMatrix*, we can still see two major families of clusters: clusters with 6, 8, 9, and 10 on one side and 1, 2, 3, 4, 5, and 7, on the other. Almost all clusters have a high value of similarity across all clustering 
solutions. Only cluster 6 has a quite low similarity value. Red color on the diagonal means that the group is homogenous, and usually, it is what we want to get. The yellow on the diagonal indicates that either 
...
# Gb9, marker gene for cluster 6
plotGeneExpression("Gb9", experimentName, outputDirectory, sceObjectCONCLUS, 
                   tSNEpicture = 10, returnPlot=T)
```
\newline
