...
if(!requireNamespace("conclus"))
	install_github("PolinaPavlovich/CONCLUS", dependencies = TRUE)
```
...
The wrapper function runCONCLUS is organized into X steps:
...
plotGeneExpression("Cd34", experimentName, outputDirectory, sceObjectCONCLUS, tSNEpicture = 10, returnPlot=T)
```

```{r}
# Ctsc, marker gene for cluster 5
plotGeneExpression("Ctsc", experimentName, outputDirectory, sceObjectCONCLUS, tSNEpicture = 10, returnPlot=T)
```

```{r}
# H19, marker gene for cluster 8
plotGeneExpression("H19", experimentName, outputDirectory, sceObjectCONCLUS, tSNEpicture = 10, returnPlot=T)
```

```{r}
# Postn, marker gene for cluster 6
plotGeneExpression("Postn", experimentName, outputDirectory, sceObjectCONCLUS, tSNEpicture = 10, returnPlot=T)
```
...
```{r}
exportClusteringResults(sceObjectCONCLUS, outputDirectory, experimentName, "clusters_table.tsv")
clustersTable <- read.delim(file.path(outputDirectory, "output_tables", paste0(experimentName, "_clusters_table.tsv")), stringsAsFactors = FALSE) 
...
write.table(clustersTable, file.path(outputDirectory, "output_tables", paste0(experimentName, "_clusters_table_manual.tsv")), quote = FALSE, sep = "\t")
```
