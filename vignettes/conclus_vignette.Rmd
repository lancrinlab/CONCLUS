...
!! integrate exportData to the vignette
...
#testClustering(sceObject, outputDirectory, experimentName)

# 3. Running the analysis and saving the consensus clustering solution
sceObjectCONCLUS <- conclus::runCONCLUS(sceObject, outputDirectory, experimentName, 
                               plotPDFcellSim = TRUE, # FALSE for > 2500 cells
                               k = 10,
...
conclus::exportClusteringResults(sceObjectCONCLUS, outputDirectory, experimentName, 
                        "clusters_table.tsv")
```
...
```{r}
clustersTable <- read.delim(file.path(outputDirectory, "output_tables",                                                    paste0(experimentName, "_clusters_table.tsv")), 
                            stringsAsFactors = FALSE)
clustersTable$clusters[clustersTable$clusters == "9"] <- "newCluster"
clustersTable$clusters[clustersTable$clusters == "10"] <- "newCluster"
write.table(clustersTable, file.path(outputDirectory, "output_tables",                                                    paste0(experimentName, "_clusters_table.tsv")), 
            quote = FALSE, sep = "\t")
```
...
sceObjectCONCLUS <- conclus::runCONCLUS(sceObjectCONCLUS, outputDirectory, experimentName, 
                        preClustered = TRUE,
                        tSNEalreadyGenerated = TRUE, # to use t-SNE coords from outputDirectory/tsnes
...
conclus::plotGeneExpression("Ccl3", experimentName, outputDirectory, 
                   sceObject = sceObjectCONCLUS,
                   tSNEpicture = 10, returnPlot = TRUE)
conclus::plotGeneExpression("Lama1", experimentName, outputDirectory, 
                   sceObject = sceObjectCONCLUS,
                   tSNEpicture = 10, returnPlot = TRUE)
tSNEstate[[10]]
```
...
The result will be saved into the "outputDirectory/marker_genes/saveGenesInfo" directory. 
```{r}
...
```{r}
saveRDS(sceObjectCONCLUS, file.path(outputDirectory, paste0("output_tables/", 
...
clusteringResults <- conclus::runClustering(tSNEResults, sceObject, outputDirectory, 
                                       experimentName,
